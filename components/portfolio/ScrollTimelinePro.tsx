...
type TimelineItem = { year: string; label: string; description: string; background: string; foreground: string };
...
function range(value: number, from: number, to: number) { if (from === to) return value >= to ? 1 : 0; return clamp01((value - from) / (to - from)); }
...
function TimelinePanel({ item, index, progress, reducedMotion }: { item: TimelineItem; index: number; progress: MotionValue<number>; reducedMotion: boolean }) {
  ...
  const yearX = useTransform(progress, (value) => {
    if (index === 0) return reducedMotion ? 0 : -range(value, 0, 1 / (count - 1)) * 1.5;
    if (index === count - 1) { const local = range(value, transitionStart, transitionEnd); return reducedMotion ? 0 : (1 - local) * 1.5; }
    if (value < transitionEnd) { const local = range(value, transitionStart, transitionEnd); return reducedMotion ? 0 : (1 - local) * 1.5; }
    return reducedMotion ? 0 : -range(value, nextStart, nextEnd) * 1.5;
  });

  const yearRotate = useTransform(progress, (value) => {
    if (index === 0) return reducedMotion ? 0 : -range(value, 0, 1 / (count - 1)) * 30;
    if (index === count - 1) { const local = range(value, transitionStart, transitionEnd); return reducedMotion ? 0 : (1 - local) * 8; }
    if (value < transitionEnd) { const local = range(value, transitionStart, transitionEnd); return reducedMotion ? 0 : (1 - local) * 8; }
  ...
  return (
    <motion.article className="absolute inset-0 overflow-hidden" style={{ backgroundColor: item.background, color: item.foreground, clipPath: reducedMotion ? (index === 0 ? 'inset(0)' : 'inset(0 0 0 100%)') : clipPath, zIndex: index + 1 }}>
      <div className="absolute inset-0">
        ...
        <motion.div className="pointer-events-none absolute bottom-[-0.025em] left-0 w-full overflow-visible px-0 text-center whitespace-nowrap font-black leading-[0.72] tracking-[-0.08em] text-[clamp(5rem,18vw,34rem)] sm:text-[clamp(8rem,27vw,34rem)]" style={{ x: yearTransformX, rotate: yearRotate, scale: yearScale, transformOrigin: '50% 50%' }}>
          {item.year}
        </motion.div>
      </div>
    </motion.article>
