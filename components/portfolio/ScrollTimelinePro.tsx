...
function TimelinePanel({
  ...
  // Each new panel is revealed from the right. At 50% scroll progress
  // it occupies exactly the right half, matching the reference design.
  const clipLeft = useTransform(progress, (value) => {
    if (index === 0) return 0;
    const local = range(value, transitionStart, transitionEnd);
    return reducedMotion ? 0 : 100 - local * 100;
  });

  // The huge year enters upright, then rotates and drifts toward the left
  // while the next color panel wipes across it.
  const yearX = useTransform(progress, (value) => {
    if (index === 0) {
      const local = range(value, 0, 1 / (count - 1));
      return reducedMotion ? 0 : -local * 18;
    }
    ...
    const local = range(value, nextStart, nextEnd);
    return reducedMotion ? 0 : -local * 18;
  });
  ...
  const yearRotate = useTransform(progress, (value) => {
    if (index === 0) {
      const local = range(value, 0, 1 / (count - 1));
      return reducedMotion ? 0 : -local * 34;
    }
      ...
      style={{
        ...
        clipPath: reducedMotion
          ? index === 0
            ? 'inset(0 0 0 0)'
            : 'inset(0 0 0 100%)'
          : useTransform(clipLeft, (left) => `inset(0 0 0 ${left}%)`),
        zIndex: index + 1,
      }}
    >
      <div className="absolute inset-0">
        ...
        <div className="absolute left-0 top-0 w-full px-7 pt-10 sm:px-12 sm:pt-14 lg:px-20 lg:pt-16">
          <div className="max-w-[680px]">
            ...
            <p className="mt-4 max-w-[760px] text-[clamp(1.25rem,3vw,2.15rem)] font-normal leading-[1.03] tracking-[-0.035em]">
              {item.description}
            </p>
          </div>
        </div>
        ...
        <motion.div
          ...
          style={{
            x: useTransform(yearX, (value) => `${value}%`),
            rotate: yearRotate,
            scale: yearScale,
